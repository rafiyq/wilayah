...
fn test_locate_contained_inside_polygon() {
    ...
    let village: Option<wilayah::Village> = conn
        .query_row(
    ...
    let village = village.expect("village id=1 should exist");
    let vlat = village.lat;
    let vlon = village.lon;
    ...
    assert_eq!(loc.village_code, village.code);
}
